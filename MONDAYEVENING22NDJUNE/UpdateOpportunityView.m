...
//    self.PPLTextField.text= [[self.appdelegate.OppurtunityList objectAtIndex:self.appdelegate.optyrow] valueForKey:@"PPL"];
    ...
    if ([appdelegate.fromManageOptyview isEqualToString:@"fromManageOptyview"])
    {
        ...
        if ([self.PLTextField respondsToSelector:@selector(setAttributedPlaceholder:)]) {
            ...
            self.PLTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[[appdelegate.OppurtunityList objectAtIndex:appdelegate.Manageoptyrow] valueForKey:@"PRODUCT_NAME1"] attributes:@{NSForegroundColorAttributeName: color}];
        } else {
        ...
        if ([self.PPLTextField respondsToSelector:@selector(setAttributedPlaceholder:)]) {
            UIColor *color = [UIColor darkGrayColor];
            self.PPLTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[[appdelegate.OppurtunityList objectAtIndex:appdelegate.Manageoptyrow] valueForKey:@"PRODUCT_NAME"] attributes:@{NSForegroundColorAttributeName: color}];
        } else {
            NSLog(@"Cannot set placeholder text's color, because deployment target is earlier than iOS 6.0");
            // TODO: Add fall-back code to set placeholder color.
        }
    ...
    else if ([appdelegate.fromSearchOptyview isEqualToString:@"fromSearchOptyview"])
    {
        ...
        if ([self.PLTextField respondsToSelector:@selector(setAttributedPlaceholder:)]) {
            UIColor *color = [UIColor darkGrayColor];
            self.PLTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[[SearchOpportunityListArray objectAtIndex:appdelegate.optyrow] valueForKey:@"PRODUCT_NAME1"] attributes:@{NSForegroundColorAttributeName: color}];
        } else {
            NSLog(@"Cannot set placeholder text's color, because deployment target is earlier than iOS 6.0");
            // TODO: Add fall-back code to set placeholder color.
        }
        if ([self.PPLTextField respondsToSelector:@selector(setAttributedPlaceholder:)]) {
            UIColor *color = [UIColor darkGrayColor];
            self.PPLTextField.attributedPlaceholder = [[NSAttributedString alloc] initWithString:[[SearchOpportunityListArray objectAtIndex:appdelegate.optyrow] valueForKey:@"PRODUCT_NAME"] attributes:@{NSForegroundColorAttributeName: color}];
        } else {
            NSLog(@"Cannot set placeholder text's color, because deployment target is earlier than iOS 6.0");
            // TODO: Add fall-back code to set placeholder color.
        }
...
//        
        ...
        self.ApplicationTextField.text=Application;
            _btnSave.enabled = YES; //Abhishek
            _btnSave.backgroundColor = [UIColor colorWithRed:(115/255.0)green:(182/255.0) blue:(68/255.0) alpha:1]; //Abhishek
        }
//
...
-(void)UpdateOptyNotificationFound:(NSNotification*)notification
{
    ...
    {
        UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:@"Oppurtunity Failed To Update" message:nil delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        [alertView show];
    }
    else if(IntegrationId_string)
    {
        UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:@"Oppurtunity Updated Successfully!" message:nil delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
        alertView.tag=20;
        [alertView show];
       ...
       // appdelegate.accountPhoneNumber= self.Phone_Number_txt_field.text;
        ...
        NSNotification *UpdateOppurtunityValues = [NSNotification notificationWithName:@"UpdateOppurtunityValues" object:nil userInfo:nil];
